...
export class VSOCapacityEventSource implements Calendar_Contracts.IEventSource {
        ...
        return this.getEvents().then((events: Calendar_Contracts.CalendarEvent[]) => {
            $.each(events, (index: number, event: Calendar_Contracts.CalendarEvent) => {
                ...
                if (start.getHours() !== 0) {
                    ...
                    var eventInArray: Calendar_Contracts.CalendarEvent = $.grep(events, function(e: Calendar_Contracts.CalendarEvent) {
                    ...
                    (iteration: Work_Contracts.TeamSettingsIteration, index: number, array: Work_Contracts.TeamSettingsIteration[]) => {
                        ...
                        iterationTeamDaysOffPromises[
                            ...
                            if (teamDaysOff && teamDaysOff.daysOff && teamDaysOff.daysOff.length) {
                                teamDaysOff.daysOff.forEach(
                                    (daysOffRange: Work_Contracts.DateRange, i: number, array: Work_Contracts.DateRange[]) => {
                                        var event: any = {};
    ...
    public updateEvent(
        ...
        if (isTeam) {
            this._getTeamDaysOff(workClient, teamContext, iterationId).then((teamDaysOff: Work_Contracts.TeamSettingsDaysOff) => {
                var teamDaysOffPatch: Work_Contracts.TeamSettingsDaysOffPatch = { daysOff: teamDaysOff.daysOff };
                var updated: boolean = teamDaysOffPatch.daysOff.some(
    ...
    public getTitleUrl(webContext: WebContext): IPromise<string> {
        ...
        workClient.getTeamIterations(teamContext, "current").then(
            (iterations: Work_Contracts.TeamSettingsIteration[]) => {
                if (iterations.length > 0) {
                    var iterationPath = iterations[0].path.substr(iterations[0].path.indexOf("\\") + 1);
                    deferred.resolve(
                        webContext.host.uri + webContext.project.name + "/" + webContext.team.name + "/_backlogs/capacity/" + iterationPath,
                    );
                } else {
    ...
    private _getTeamDaysOff(
        workClient: Work_Client.WorkHttpClient,
        teamContext: TFS_Core_Contracts.TeamContext,
        iterationId,
    ): IPromise<Work_Contracts.TeamSettingsDaysOff> {
        var deferred = Q.defer<Work_Contracts.TeamSettingsDaysOff>();
    ...
    private _getCapacity(
        ...
        iterationId,
        memberId: string,
    ): IPromise<Work_Contracts.TeamMemberCapacity> {
        var deferred = Q.defer<Work_Contracts.TeamMemberCapacity>();
    ...
    private _getCategorySubTitle(
        ...
        category.events.forEach(e => {
            ...
            var datesInRange = Calendar_DateUtils.getDatesInRange(
...
export class IdentityHelper {
    ...
    public static parseUniquefiedIdentityName(name: string): string {
        ...
        name = name
            .replace(IdentityHelper.AAD_IDENTITY_UNIQUEFIEDNAME_SEPERATOR_START, IdentityHelper.IDENTITY_UNIQUEFIEDNAME_SEPERATOR_START)
